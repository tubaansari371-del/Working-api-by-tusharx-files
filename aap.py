...
# 🔄 UPDATED: Ab ye function bina kisi dict ya uid ke, direct strings (tokens) ki list read karega
def load_tokens(server_name, for_visit=False):
    ...
    try:
        with open(path, "r") as f:
            tokens = json.load(f)
            # Ab ye check karega ki JSON ek string list hai ya nahi
            if isinstance(tokens, list) and all(isinstance(t, str) for t in tokens):
                ...
                print(f"Warning: Token file {path} is not a valid list of strings. Returning empty list.")
                return []
...
# 🔄 UPDATED: Token dictionary ki jagah ab direct string token use karega
async def send_single_like_request(encrypted_like_payload, token_value, url):
    edata = bytes.fromhex(encrypted_like_payload)
    if not token_value:
        print("Warning: send_single_like_request received an empty token.")
        return 999
...
async def send_likes_with_token_batch(uid, server_region_for_like_proto, like_api_url, token_batch_to_use):
    ...
    tasks = []
    for token_value_for_request in token_batch_to_use:
        tasks.append(send_single_like_request(encrypted_like_payload, token_value_for_request, like_api_url))
...
# 🔄 UPDATED: Profile check request ke andar bhi ab token direct string ke roop mein ja raha hai
def make_profile_check_request(encrypted_profile_payload, server_name, token_value):
    if not token_value:
        print("Warning: make_profile_check_request received an empty token.")
        return None
